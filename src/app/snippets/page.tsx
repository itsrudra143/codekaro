...
function SnippetsPage() {
  ...
  useEffect(() => {
    const fetchSnippets = async () => {
      ...
      try {
        ...
        const formattedData = data.map(snippet => ({
          ...snippet,
        ...
        toast.error(`Failed to load snippets: ${errorMessage}`);
        setSnippets([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const languages = [...new Set(safeSnippets.map((s) => s.language))];
  ...
  const filteredSnippets = safeSnippets.filter((snippet) => {
    ...
    const matchesSearch =
      ...
      snippet.userName.toLowerCase().includes(lowerSearch);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-12">
          ...
          >
            <div className="text-center">
              ...
              <p className="text-gray-400 mb-6">
                Try adjusting your search query or clearing the language filter.
              </p>
        ...
        {/* Initial Empty State (no snippets at all) */}
        {!isLoading && !error && safeSnippets.length === 0 && (
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="relative max-w-md mx-auto mt-20 p-8 rounded-2xl overflow-hidden"
