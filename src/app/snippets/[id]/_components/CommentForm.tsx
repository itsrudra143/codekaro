...
function CommentForm({ isSubmitting, onSubmit }: CommentFormProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Tab" && !e.shiftKey) {
      ...
      // Use requestAnimationFrame to ensure cursor position is updated after state change
      ...
      <div className="bg-[#0a0a0f] rounded-xl border border-[#ffffff0a] overflow-hidden">
        ...
        {isPreview ? (
          <div className="min-h-[120px] p-4 text-[#e1e1e3]">
            {/* Ensure CommentContent is robust enough for empty strings */}
            <CommentContent content={comment || "Nothing to preview."} />
          </div>
        ) : (
          <textarea
            value={comment}
            ...
            placeholder="Add to the discussion... Supports Markdown for **bold**, *italic*, and ```code blocks```."
            rows={5} // Suggest a sensible default height
            className="w-full bg-transparent border-0 text-[#e1e1e3] placeholder:text-[#808086] outline-none 
            resize-none min-h-[120px] p-4 font-mono text-sm focus:ring-0" // Add focus:ring-0 to remove potential default focus rings
          />
        )}
